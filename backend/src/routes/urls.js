...
router.post('/shorten', async (req, res) => {
  try {
    ...
    if (customAlias) {
      ...
      if (aliasClean.length < 3) return res.status(400).json({ error: 'Alias too short' });
...
router.get('/links', async (req, res) => {
  try {
    ...
    const baseUrl = process.env.BASE_URL || 'http://localhost:5000'\;
    res.json({ links: urls.map(function(u) { return { shortUrl: baseUrl + '/' + u.shortCode, shortCode: u.shortCode, originalUrl: u.originalUrl, clicks: u.clicks, expiresAt: u.expiresAt, createdAt: u.createdAt }; }), total, page, pages: Math.ceil(total / limit) });
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
  }
});
